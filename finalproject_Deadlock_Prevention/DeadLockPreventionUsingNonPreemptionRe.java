...
public class DeadLockPreventionUsingNonPreemptionRe extends Thread {
	// 스레드의 id
	private int id; 
	// 공유 자원의 개수
	...
	public int run = 0; 
	static private Map<Semaphore, Thread> checkList = new HashMap<>();

	public DeadLockPreventionUsingNonPreemptionRe(int id, int numOfResource, List<Semaphore> resource, int maxResource,
			int sleepTime) {
		this.id = id;
		this.resource = resource;
		this.numOfResource = numOfResource;
		lock = new boolean[numOfResource];
			...
			try {
				...
				while (require < rand) {
					this.save.forEach((k, v) -> {
						if (v.tryAcquire()) {
							checkList.put(v, Thread.currentThread());
							require++;
						} else {
							// 잠겨있지 않으면 뺏어옴
							if (!lock[k]) {
							...
							// 잠겨있으면 확보했던 자원들을 놓고 처음으로 돌아감.
								...
								checkList.forEach((kkk,vvv) -> {
										if(vvv == Thread.currentThread())
											kkk.release();
										});
								require = 0;
								return;
					...
					for(Integer key : save.keySet()){
						if(checkList.get(save.get(key)) != Thread.currentThread()){
							checkList.forEach((kkk,vvv) -> {
								if(vvv == Thread.currentThread())
									kkk.release();
								});
